...
class HUD extends FlxTypedGroup<FlxSprite>
{
	...
	public function new()
	{
		...
		// Health image
		_sprHealth = new FlxSprite(4,
			_txtHealth.y + (_txtHealth.height / 2) - 4, AssetPaths.health__png);
		// Money image
		_sprMoney = new FlxSprite(FlxG.width - 12,
			_txtMoney.y + (_txtMoney.height / 2) - 4, AssetPaths.coin__png);
		_txtMoney.alignment = RIGHT;
		_txtMoney.x = _sprMoney.x - _txtMoney.width - 4;

		// Add FlxSprite objects
		add(_sprBack);
		add(_sprHealth);
		add(_sprMoney);
		add(_txtHealth);
