...
class OptionsState extends FlxState
{
	...
	override public function create():Void 
	{
		...
		_btnVolumeDown = new FlxButton(8, _txtVolume.y + _txtVolume.height + 2, "-",
			clickVolumeDown);
		...
		_barVolume = new FlxBar(_btnVolumeDown.x + _btnVolumeDown.width + 4, _btnVolumeDown.y,
			LEFT_TO_RIGHT, Std.int(FlxG.width - 64), Std.int(_btnVolumeUp.height));
		...
		_btnClearData = new FlxButton((FlxG.width / 2) - 90, FlxG.height - 28,
			"Clear Data", clickClearData);
	...
	/**
	 * The user wants to clear the saved data - we just call erase on our
	 *   save object and then reset the volume to .5
	 */
	...
	/**
	 * The user clicked the back button - close our save object,
	 *   and go back to the MenuState
	 */
	...
	/**
	 * The user clicked the down button for volume -
	 *   we reduce the volume by 10% and update the bar
	 */
	...
	/**
	 * The user clicked the up button for volume -
	 *   we increase the volume by 10% and update the bar
	 */
	...
	/**
	 * Whenever we want to show the value of volume,
	 *   we call this to change the bar and the amount text
	 */
